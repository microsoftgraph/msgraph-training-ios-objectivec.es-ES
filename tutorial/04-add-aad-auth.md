...
1. Abra **AuthenticationManager. h** y agregue la siguiente `#import` instrucción en la parte superior del archivo.

    ```objc
    #import <MSGraphMSALAuthProvider/MSGraphMSALAuthProvider.h>
    ```

1. Agregue la siguiente línea dentro de `@interface` la declaración.

    ```objc
    - (MSALAuthenticationProvider*) getGraphAuthProvider;
    ```

1. Abra **AuthenticationManager. m** y agregue la siguiente función a la `AuthenticationManager` clase.
